 ...
 */
template <class T> void appendVia(T& headers, const std::string& via) {
 ...
 * - Upgrade: [any value]
 */
bool isUpgradeWorker(const HeaderEntry* connection, const HeaderEntry* upgrade);
template <class T> bool isUpgrade(const T& headers) {
  return isUpgradeWorker(headers.Connection(), headers.Upgrade());
}
